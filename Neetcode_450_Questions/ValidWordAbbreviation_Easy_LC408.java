...
*/
public class ValidWordAbbreviation_Easy_LC408 {
    class Solution {
        ...
        public boolean validWordAbbreviation(String word, String abbr) {
            ...
            while (wordIdx < word.length() && abbrIdx < abbr.length()) {
                if (Character.isDigit(abbr.charAt(abbrIdx))) {
                    // Check for starting zero
                    if ((abbr.charAt(abbrIdx) - '0') == 0) {
                        return false;
                    }
                    int k = abbrIdx;
                    ...
                    int len = Integer.parseInt(abbr.substring(abbrIdx, k));
                    // Jump by k steps
                    wordIdx = wordIdx + len;
                    abbrIdx = k;

                } else {
                    // If not digit we compare
